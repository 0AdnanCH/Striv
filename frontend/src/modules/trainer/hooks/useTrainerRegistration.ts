...
import type { IdentityInfoPayload, PersonalInfoPayload, ProfessionalInfoPayload, TrainerFullInfo, WorkInfoPayload } from '../types/trainerRegistration.types';
...
export function useTrainerRegistration() {
  ...
  const submitIdentityInfo = async (data: IdentityInfoPayload) => {
    try {
      ...
      setTrainerFullInfo((prev) => {
        ...
        console.log(res.data.documentType)
        return {
          ...prevData,
          kyc: {
            ...prevData.kyc,
  ...
  const fetchTrainerFullInfo = async (): Promise<TrainerFullInfo> => {
    try {
      setLoading(true);
